...
class Reader {
  ...
  getNumber() {
    ...
    if (this.ch == CH['0']) {
      ...
      if (this.ch == CH.x || this.ch == CH.X) {
        this.getChar();
        return new Token(TK.CINT, this.getHex(), currentLine);
      } else {
        return new Token(TK.CINT, this.getOctal(), currentLine);
      }
    } else {
      const num = this.getDecimal();
      if (num % 1 > 0)
  ...
  getDotOrEllipsisOrNumber() {
    ...
    if (this.ch === CH["."]) {
      ...
      return new Token(TK.CFLOAT, this.getDecimalRev(), currentLine);
    } else {
