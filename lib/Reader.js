const {
  ...
  checkNumber, checkDecimal, checkOctal, checkHex, hexToNum,
} = require("./utils");
...
class Reader {
  ...
  /**
   * A decimal number shouldn't contains more than 1 dot.
   * @returns {String}
   */
  getDecimalBase() {
    const currentLine = this.line;
    ...
    if (ret.indexOf(".") !== ret.lastIndexOf("."))
      throw new CLexicalError(`Invalid float number`, currentLine);
   ...
   */
  getDecimalNum() {
    return Number(this.getDecimalBase());
  }
  ...
  getOctalNum() {
    ...
    while (checkOctal(this.getChar()))
      ret += String.fromCharCode(this.ch);

    return Number("0o" + ret);
  }
  ...
  /**
   * Decimal could be Integer or float number.
   * @returns {Token}
   */
  getDecimal() {
    const currentLine = this.line;
    const num = this.getDecimalNum();
    if (num % 1 > 0)
  ...
  /**
   * Hexadecimal starts with "0x", octal starts with "0", others are decimal.
   */
  getNumber() {
    const currentLine = this.line;
    var ret = "";
    if (this.ch == CH['0']) {
      this.getChar();
      if (this.ch == CH.x || this.ch == CH.X) {
        this.getChar();
        return new Token(TK.CINT, this.getHexNum(), currentLine);
      } else {
        return new Token(TK.CINT, this.getOctalNum(), currentLine);
      }
    } else {
  ...
  getDotOrEllipsisOrNumber() {
    ...
    } else if (checkDecimal(this.ch)) {
      return new Token(TK.CFLOAT, this.getDecimalRevNum(), currentLine);
    } else {
